...
export async function getInsights(userId) {
  const forms = await formRepo.listFormsByOwner(userId, { filters: "all" });
  ...
  const topForms = [...forms]
    ...
    .map((f) => ({
      ...
      color: f.settings?.primaryColor || "#8c8b7c",
    }));

  return {
    stats: { totalForms: forms.length, published, totalResponses, totalViews, conversion, avgCompletionTime },
    timeline,
    heatmap,
    devices,
    topForms,
...
export async function getInbox(userId, { search = "" } = {}) {
  ...
  let responses = await responseRepo.recentByForms(forms.map((f) => f._id), 300);
  ...
  return responses.map((r) => {
    ...
    return {
      ...
      formColor: form?.settings?.primaryColor || "#8c8b7c",
    };
  });
}