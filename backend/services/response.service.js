...
export async function submitResponse(slug, { answers = [], completionTime = 0, meta = {} }) {
...
function validateAnswer(q, value) {
  ...
  if (q.type === "email" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(asString)) {
    throw ApiError.badRequest(v.message || `"${q.label}" must be a valid email`);
  }
  ...
  if (q.type === "number") {
    ...
    if (Number.isNaN(num)) throw ApiError.badRequest(`"${q.label}" must be a number`);
    if (v.min != null && num < v.min) throw ApiError.badRequest(v.message || `"${q.label}" is too small`);
    if (v.max != null && num > v.max) throw ApiError.badRequest(v.message || `"${q.label}" is too large`);
  }
  if (v.pattern) {
    try {
  ...
  if (OPTION_FIELD_TYPES.includes(q.type) && q.options.length) {
    ...
    for (const sel of selected) {
      if (!allowed.has(sel)) {
        throw ApiError.badRequest(`"${sel}" is not a valid choice for "${q.label}"`);
      }
    }
  }
}
