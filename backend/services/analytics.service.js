...
export async function getFormAnalytics(formId, userId) {
  ...
  const avgCompletionTime = totalResponses
    ? Math.round(responses.reduce((sum, r) => sum + (r.completionTime || 0), 0) / totalResponses)
    : 0;
...
function computeCompletionRate(responses, answerableCount) {
  ...
  const total = responses.reduce(
    (sum, r) => sum + Math.min(r.answers.length, answerableCount) / answerableCount,
    0
  );
...
function buildQuestionAnalytics(form, responses) {
  return form.questions
    ...
    .map((q) => {
      ...
      const base = { id: q.id, label: q.label, type: q.type, total: values.length };
      ...
      if (OPTION_FIELD_TYPES.includes(q.type)) {
        ...
        return {
          ...base,
          breakdown: [...tally.entries()].map(([label, count]) => ({ label, count })),
        };
      }
