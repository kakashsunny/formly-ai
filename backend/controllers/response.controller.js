...
export const getResponses = asyncHandler(async (req, res) => {
  const responses = await responseService.listResponses(req.params.id, req.user.id, {
...
export const getAnalytics = asyncHandler(async (req, res) => {
  const analytics = await getFormAnalytics(req.params.id, req.user.id);
  sendSuccess(res, { data: { analytics } });
});

export const deleteResponse = asyncHandler(async (req, res) => {
  await responseService.deleteResponse(req.params.id, req.user.id);
  sendSuccess(res, { message: "Response deleted" });
});

export const exportResponses = asyncHandler(async (req, res) => {
  const { form, responses } = await responseService.exportResponses(req.params.id, req.user.id);
