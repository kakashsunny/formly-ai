...
export const getForms = asyncHandler(async (req, res) => {
  ...
  const forms = await formService.listForms(req.user.id, { search, filter });
...
export const updateForm = asyncHandler(async (req, res) => {
  const form = await formService.updateForm(req.params.id, req.user.id, req.body);
...
export const publishForm = asyncHandler(async (req, res) => {
  ...
  const form = await formService.setPublishState(req.params.id, req.user.id, shouldPublish);
  sendSuccess(res, {
...
export const duplicateForm = asyncHandler(async (req, res) => {
  const form = await formService.duplicateForm(req.params.id, req.user.id);
  sendSuccess(res, { statusCode: 201, message: "Form duplicated", data: { form } });
});

export const deleteForm = asyncHandler(async (req, res) => {
  await formService.deleteForm(req.params.id, req.user.id);
